...
import ProductPaymentsTable from "./payments.js";
...
export default class PayProductDebt extends Form {
  constructor() {
    super();
  }
  ...
  debtDetails(element, debt) {
    ...
    product_name.classList = "debt-product-name";
    product_name.textContent = debt.product;
    const product_category = document.createElement("div");
    product_category.classList = "debt-product-category";
    product_category.textContent = debt.category;
    const product_quantity = document.createElement("div");
    product_quantity.classList = "debt-product-quantity";
    product_quantity.textContent = `${debt.quantity.value} ${debt.quantity.unit}`;
    const creditor = document.createElement("div");
    creditor.classList = "debt-creditor";
    creditor.textContent = `${debt.creditor.name} - ${debt.creditor.location.region}, 
    ${debt.creditor.location.county} (${debt.creditor.location.country})`;
    ...
    if (debt.payment.status === "pending") {
      ...
      amount.textContent = `KSh ${debt.price.total}`;
      container.append(amount);
    }
  ...
  debtNextPayment(element, debt) {
    ...
    if (debt.payment.status === "pending") {
      input.value = debt.payment.balance;
    } else if (debt.payment.status === "completed") {
      input.value = debt.price.total;
    }
  ...
  debtLastPayments(element, debt) {
    const payments_table = new ProductPaymentsTable(debt.payment.last_payment);
